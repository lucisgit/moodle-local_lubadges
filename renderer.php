...
/**
 * Renderer for use with the LU Badges output.
 *
 ...
 */
class local_lubadges_renderer extends plugin_renderer_base {
    ...
    public function print_badge_overview($badge, $context) {
        ...
        if ($badge->has_criteria()) {
            ...
            if (has_capability('moodle/badges:configurecriteria', $context)) {
                $display .= $this->output->single_button(
                    new moodle_url('/local/lubadges/criteria.php', array('id' => $badge->id)),
        ...
        if (has_capability('moodle/badges:viewawarded', $context)) {
            ...
            if ($badge->has_awards()) {
                $url = new moodle_url('/local/lubadges/recipients.php', array('id' => $badge->id));
                $a = new stdClass();
                $a->link = $url->out();
            ...
            if (has_capability('moodle/badges:awardbadge', $context) &&
                ...
                $badge->is_active()) {
                $display .= $this->output->single_button(
                        new moodle_url('/local/lubadges/award.php', array('id' => $badge->id)),
    ...
    public function print_badge_table_actions($badge, $context) {
        ...
        if (has_capability('moodle/badges:configuredetails', $context) && $badge->has_criteria()) {
            ...
            if ($badge->status == BADGE_STATUS_INACTIVE || $badge->status == BADGE_STATUS_INACTIVE_LOCKED) {
                ...
                $url = new moodle_url('/local/lubadges/action.php');
        ...
        if ($badge->has_manual_award_criteria() &&
                ...
                $badge->is_active()) {
            $url = new moodle_url('/local/lubadges/award.php', array('id' => $badge->id));
            $actions .= $this->output->action_icon($url, new pix_icon('t/award', get_string('award', 'badges'))) . " ";
        }

        // Edit badge.
        if (has_capability('moodle/badges:configuredetails', $context)) {
            $url = new moodle_url('/local/lubadges/edit.php', array('id' => $badge->id, 'action' => 'details'));
    ...
    protected function render_badge_management(badge_management $badges) {
        ...
        foreach ($badges->badges as $b) {
            ...
            $name = html_writer::link(new moodle_url('/local/lubadges/overview.php', array('id' => $b->id)), $forlink, $style);
            $status = $b->statstring;
            $criteria = self::print_badge_criteria($b, 'short');

            if (has_capability('moodle/badges:viewawarded', $this->page->context)) {
                $awards = html_writer::link(new moodle_url('/local/lubadges/recipients.php', array('id' => $b->id)), $b->awards);
            } else {
                $awards = $b->awards;
            }
        ...
        return $htmladd . $htmlpagingbar . $htmltable . $htmlpagingbar;
    }
    ...
    public function print_badge_tabs($badgeid, $context, $current = 'overview') {
        ...
        $row[] = new tabobject('overview',
                    new moodle_url('/local/lubadges/overview.php', array('id' => $badgeid)),
                    get_string('boverview', 'badges')
                );

        if (has_capability('moodle/badges:configuredetails', $context)) {
            $row[] = new tabobject('details',
                        new moodle_url('/local/lubadges/edit.php', array('id' => $badgeid, 'action' => 'details')),
        ...
        if (has_capability('moodle/badges:configurecriteria', $context)) {
            $row[] = new tabobject('criteria',
                        new moodle_url('/local/lubadges/criteria.php', array('id' => $badgeid)),
                        get_string('bcriteria', 'badges')
                    );
        }
        ...
        if (has_capability('moodle/badges:configuremessages', $context)) {
            $row[] = new tabobject('message',
                        new moodle_url('/local/lubadges/edit.php', array('id' => $badgeid, 'action' => 'message')),
        ...
        if (has_capability('moodle/badges:viewawarded', $context)) {
            ...
            $row[] = new tabobject('awards',
                        new moodle_url('/local/lubadges/recipients.php', array('id' => $badgeid)),
    ...
    public function print_badge_status_box(badge $badge) {
        if (has_capability('moodle/badges:configurecriteria', $badge->get_context())) {
            ...
            if (!$badge->has_criteria()) {
                $criteriaurl = new moodle_url('/local/lubadges/criteria.php', array('id' => $badge->id));
                ...
                if ($badge->is_active()) {
                    $action = $this->output->single_button(new moodle_url('/local/lubadges/action.php',
        ...
        if (!$badge->is_active() && !$badge->is_locked()) {
            ...
            if (!empty($potential)) {
                ...
                $output .= $this->output->single_select(
                    new moodle_url('/local/lubadges/criteria_settings.php', array('badgeid' => $badge->id, 'add' => true)),
                    'type',
                    $select,
                    '',
                    array('' => 'choosedots'),
