...
/**
 * Page to handle actions associated with LU Badges management.
 *
...
$PAGE->set_url('/local/lubadges/action.php', array('id' => $badge->id));

if ($return !== 0) {
    $returnurl = new moodle_url($return);
} else {
    $returnurl = new moodle_url('/local/lubadges/overview.php', array('id' => $badge->id));
}
$returnurl->remove_params('awards');

if ($copy) {
    require_sesskey();
    require_capability('local/lubadges:addbadge', $context);

    $cloneid = $badge->make_clone();
    // If a user can edit badge details, they will be redirected to the edit page.
    if (has_capability('moodle/badges:configuredetails', $context)) {
        redirect(new moodle_url('/local/lubadges/edit.php', array('id' => $cloneid, 'action' => 'details')));
    }
    redirect(new moodle_url('/local/lubadges/overview.php', array('id' => $cloneid)));
}
...
if ($activate) {
    ...
    $url = new moodle_url('/local/lubadges/action.php', $params);
