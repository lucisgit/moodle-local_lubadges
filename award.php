...
/**
 * Handle manual LU Badge instance award.
 *
...
$url = new moodle_url('/local/lubadges/award.php', array('id' => $badgeid, 'role' => $role));
$PAGE->set_url($url);
...
if (count($acceptedroles) > 1) {
    ...
    if (!empty($selection)) {
        ...
        if (!$role) {
            $pageurl = new moodle_url('/local/lubadges/award.php', array('id' => $badgeid));
            echo $OUTPUT->header();
