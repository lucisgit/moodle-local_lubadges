...
$hdr = get_string('managebadges', 'local_lubadges');
$returnurl = new moodle_url('/local/lubadges/index.php', $urlparams);
...
if ($type == BADGE_TYPE_SITE) {
    ...
    navigation_node::override_active_url(new moodle_url('/local/lubadges/index.php', array('type' => BADGE_TYPE_SITE)), true);
} else {
    ...
    navigation_node::override_active_url(
        new moodle_url('/local/lubadges/index.php', array('type' => BADGE_TYPE_COURSE, 'id' => $course->id))
    );
}
...
if (($delete || $archive) && has_capability('moodle/badges:deletebadge', $PAGE->context)) {
    ...
    if (!$confirm) {
        ...
        // Also need to delete LU badge instance record.
        if ($archiveonly == false) {
            if ($instance = $DB->get_field('local_lubadges_instances', 'id', array('instanceid' => $badge->id))) {
                $DB->delete_records('local_lubadges_instances', array('id' => $instance));
            }
        }
        redirect($returnurl);
    }
...
$totalcount = count(badges_get_badges($type, $courseid, '', '' , 0, BADGE_PERPAGE, 0, true));
$records = badges_get_badges($type, $courseid, $sortby, $sorthow, $page, BADGE_PERPAGE, 0, true);
...
if ($totalcount) {
    ...
    if (has_capability('local/lubadges:addbadge', $PAGE->context)) {
