...
/**
 * Editing LU Badge instance details, criteria, messages.
 *
...
$currenturl = new moodle_url('/local/lubadges/edit.php', array('id' => $badge->id, 'action' => $action));
...
if ($action == 'details' and $prototype = $DB->get_field('local_lubadges_instances', 'protoid', array('instanceid' => $badgeid))) {
    $extrafields = $DB->get_record('local_lubadges_prototypes', array('id' => $prototype),
            'requirements, hint, collection, level, points');
...
if ($form->is_cancelled()) {
    redirect(new moodle_url('/local/lubadges/overview.php', array('id' => $badgeid)));
    ...
    if ($action == 'details') {
        ...
        if (!empty($extrafields)) {
            foreach (array_keys(get_object_vars($extrafields)) as $field) {
                unset($badge->$field);
            }
        }
